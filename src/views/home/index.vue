...
onMounted(() => {
  ...
  // 设置标题 - 优先使用siteTitle，其次使用logoText
  const siteTitle = panelState.panelConfig.siteTitle || panelState.panelConfig.logoText || 'Sun-Panel'
  setTitle(siteTitle)

  // 设置自定义favicon
  if (panelState.panelConfig.faviconUrl) {
    updateFavicon(panelState.panelConfig.faviconUrl)
  }
})
