...
function handleFaviconUploadFinish({
  file,
  event,
}: {
  file: UploadFileInfo
  ...
  const res = JSON.parse((event?.target as XMLHttpRequest).response)
  panelState.panelConfig.faviconUrl = res.data.imageUrl
...
<template>
  <div class="bg-slate-200 dark:bg-zinc-900 rounded-[10px] p-[8px] overflow-auto">
    ...
    <NCard style="border-radius:10px" class="mt-[10px]" size="small">
      ...
      <div class="mt-[5px]">
        <div>{{ $t('apps.baseSettings.siteTitle') }}</div>
        <div class="flex items-center mt-[5px]">
          <NInput v-model:value="panelState.panelConfig.siteTitle" type="text" show-count :maxlength="50" placeholder="Sun-Panel" clearable @update:value="onSiteConfigChange" />
        </div>
      </div>

      <div class="mt-[10px]">
        <div>{{ $t('apps.baseSettings.loginTitle') }}</div>
        <div class="flex items-center mt-[5px]">
          <NInput v-model:value="panelState.panelConfig.loginTitle" type="text" show-count :maxlength="50" :placeholder="$t('common.appName')" clearable @update:value="onSiteConfigChange" />
        </div>
      </div>

      <div class="mt-[10px]">
        <div>{{ $t('apps.baseSettings.loginFooter') }}</div>
        <div class="flex items-center mt-[5px]">
          <NInput v-model:value="panelState.panelConfig.loginFooter" type="textarea" clearable @update:value="onSiteConfigChange" />
        </div>
      </div>

      <div class="mt-[10px]">
        <div>{{ $t('apps.baseSettings.favicon') }}</div>
        <div class="flex items-center mt-[5px]">
          <NUpload
            ...
            :directory-dnd="false"
            @finish="handleFaviconUploadFinish"
          >
          ...
          <NInput v-model:value="panelState.panelConfig.faviconUrl" type="text" size="small" style="max-width: 250px;" placeholder="/favicon.svg" clearable @update:value="onSiteConfigChange" />
        </div>
      </div>
    </NCard>
